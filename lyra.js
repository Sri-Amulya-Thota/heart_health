document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateOutcomeScreen = () => {
        ...
        outcomeContent.classList.remove('hidden');
        riskCalculator.classList.add('hidden');
        privacyPolicyScreen.classList.add('hidden');
    };
    ...
    const calculateRisk = (e) => {
        ...
        if (weight === "obese") score += 2;

        let message = "";
        if (score <= 4) {
            message = "🟢 Low Risk: Your heart health risk is low. Keep up the healthy habits!";
        } else if (score <= 8) {
            message = "🟡 Moderate Risk: You have a moderate risk. Consider lifestyle changes and regular check-ups.";
        } else {
            message = "🔴 High Risk: Your heart health risk is high. Please consult a healthcare provider for personalized guidance.";
        }
    ...
    if (showCalculatorButton) {
        showCalculatorButton.addEventListener('click', () => {
            ...
            privacyPolicyScreen.classList.add('hidden');
            riskCalculator.classList.remove('hidden');
        });
    }
